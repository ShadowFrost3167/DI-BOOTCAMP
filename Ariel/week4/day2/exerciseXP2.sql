...
-- SELECT film_id, title, description, release_year, description FROM film ORDER BY rental_rate;
...
-- SELECT film_id, title, description, length, rental_rate FROM film WHERE title = 'Saturday Lambs';
...
-- SELECT * FROM film ORDER BY replacement_cost OFFSET 10;
...
-- SELECT customer.first_name, customer.last_name, payment.amount, payment.payment_date FROM customer LEFT JOIN payment ON customer.customer_id = payment.customer_id ORDER BY payment.payment_id;
-- couldn't get DISTINCT to work here, not sure why. 
